        ...
        return $table
            ->columns([
                ...
                ColumnGroup::make('Informasi Pelanggan', [
                    ImageColumn::make('data.avatar_url')
                        ->label('Avatar')
                        ->circular()
                        ->defaultImageUrl(fn($record) => CustomerHelper::generateDefaultAvatar($record))
                        ->getStateUsing(fn($record) => CustomerHelper::getFilamentAvatarUrl($record))
                    ...
                    TextColumn::make('data.name')
                        ...
                        ->getStateUsing(fn($record) => CustomerHelper::getName($record))
                        ->toggleable(isToggledHiddenByDefault: false),
                    ...
                    TextColumn::make('email')
                        ...
                        ->fontFamily('mono')
                        ->toggleable(isToggledHiddenByDefault: false),
                    IconColumn::make('data.member')
                        ->label('Member')
                        ->boolean()
                        ->alignCenter()
                        ->getStateUsing(fn($record) => CustomerHelper::isMember($record))
                        ->toggleable(isToggledHiddenByDefault: false),
                ]),
            ...
            ->filters([
                ...
                TernaryFilter::make('member')
                    ...
                    ->trueLabel('Member saja')
                    ->falseLabel('Non-member saja')
                    ->queries(
                        true: fn($query) => $query->whereRaw("data->>'member' = 'true'"),
                        false: fn($query) => $query->whereRaw("(data->>'member' = 'false' OR data->>'member' IS NULL)"),
