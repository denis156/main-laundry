    ...
    <div class="flex items-center">
        ...
        <a href="{{ route('kurir.pembayaran') }}" wire:navigate class="flex-1 flex flex-col items-center gap-1">
            <x-icon name="{{ in_array(Route::currentRouteName(), ['kurir.pembayaran', 'kurir.pembayaran.detail']) ? 'solar.wallet-money-bold' : 'solar.wallet-money-linear' }}"
                class="h-6 {{ in_array(Route::currentRouteName(), ['kurir.pembayaran', 'kurir.pembayaran.detail']) ? 'text-accent' : 'text-primary-content' }}" />
            <span class="text-xs {{ in_array(Route::currentRouteName(), ['kurir.pembayaran', 'kurir.pembayaran.detail']) ? 'text-accent' : 'text-primary-content' }} font-semibold">Pembayaran</span>
        </a>
