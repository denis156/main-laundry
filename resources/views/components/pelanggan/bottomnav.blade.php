<footer class="fixed bottom-0 left-0 right-0 bg-primary p-2 rounded-t-2xl z-50">
    <div class="flex items-center">
        <a href="{{ route('pelanggan.pesanan') }}" wire:navigate class="flex-1 flex flex-col items-center gap-1 relative">
            ...
            @if(in_array(Route::currentRouteName(), ['pelanggan.pesanan', 'pelanggan.pesanan.detail']))
        ...
        <a href="{{ route('pelanggan.profil') }}" wire:navigate class="flex-1 flex flex-col items-center gap-1 relative">
            ...
            @if(Route::currentRouteName() === 'pelanggan.profil')
                <div class="absolute -bottom-2 left-1/2 transform -translate-x-1/2 w-8 h-1 bg-accent rounded-full"></div>
            @endif
        </a>
    </div>
