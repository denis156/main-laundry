<div class="fixed bottom-6 right-6 z-50">
    ...
    <x-modal wire:model="installWebApp" title="Install Aplikasi" class="backdrop-blur-sm modal-bottom sm:modal-middle">
        <div class="flex flex-col items-center text-center space-y-6 py-4">
            ...
            {{-- Judul dan Deskripsi --}}
            <div class="space-y-2">
                <h3 class="text-2xl font-bold text-accent">Kurir <span class="text-primary">{{ $appName }}</span>
                </h3>
                <p class="text-sm text-secondary font-medium">Versi {{ $appVersion }}</p>
                <p class="text-base-content/70 max-w-sm">
                    Install aplikasi kurir ini ke perangkat Anda untuk akses yang lebih cepat dan pengalaman yang lebih
                    baik
                </p>
            </div>
        </div>
        ...
        <x-slot:actions>
            <x-button label="Install Sekarang" wire:click="closeConfirmationInstall" class="btn-accent btn-block" />
        </x-slot:actions>
    </x-modal>
</div>
